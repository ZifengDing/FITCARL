...
class baseFSLDataset(object):
    def __init__(self, datapath, statpath, ismeta=False):
        """base Dataset. Read data files and preprocess.
        Args:
            trainpath: File path of train Data;
            testpath: File path of test data;
            ...
            validpath: File path of valid data
        """
        self.allQuadruples = self.load_quadruples((datapath))
        self.num_e, self.num_r = self.get_total_number(statpath)  # number of entities, number of relations

        if ismeta == 0:
            self.skip_dict = self.get_skipdict(self.allQuadruples)
            self.skip_dict_unaware = self.get_skipdict_unaware(self.allQuadruples)

        self.train_entities = set()  # Entities that have appeared in the training set
        for query in self.allQuadruples:
            self.train_entities.add(query[0])
            self.train_entities.add(query[2])
    ...
    def get_skipdict(self, quadruples):
        """Used for time-aware filtered metrics.
        return: a dict [key -> (entity, relation, timestamp),  value -> a set of ground truth entities]
        """
        filters = defaultdict(set)
        for src, rel, dst, time in quadruples:
    ...
    def get_skipdict_unaware(self, quadruples):
        """Used for time-unaware filtered metrics.
        return: a dict [key -> (entity, relation, timestamp),  value -> a set of ground truth entities]
        """
        filters = defaultdict(set)
        for src, rel, dst, time in quadruples:
            filters[(src, rel)].add(dst)
            filters[(dst, rel+self.num_r+1)].add(src)
        return filters
...
class QuadruplesDatasetFSL(Dataset):
    def __init__(self, examples, num_r, entities):
        """
        examples: a list of quadruples.
        num_r: number of relations
        """
        ...
        # self.quadruples = examples.copy()
        for ex in examples:
            if ex[2] in entities: # few shot entities as object
                self.quadruples_.append([ex[2], ex[1]+num_r+1, ex[0], ex[3]])
            else:
                self.quadruples_.append([ex[0], ex[1], ex[2], ex[3]]) # few shot entities as subject
        self.quadruples = self.quadruples_
